...
//VMOLAB
#if USE(WEB_THREAD) || ENABLE(VMOLAB)
...
#if ENABLE(VMOLAB)
PassRefPtr<StringImpl> AtomicString::addSlowCase(AtomicStringTable& stringTable, const LChar* c)
{
    if (!c)
        return 0;
    if (!*c)
    ...
    AtomicStringTableLocker locker;
    HashSet<StringImpl*>::AddResult addResult = stringTable.table().add<CStringTranslator>(c);

    return addResult.isNewEntry ? adoptRef(*addResult.iterator) : *addResult.iterator;
}

PassRefPtr<StringImpl> AtomicString::addSlowCase(AtomicStringTable& stringTable, const LChar* s, unsigned length)
{
    if (!s)
        return 0;
    if (!length)
    ...
    AtomicStringTableLocker locker;
    HashSet<StringImpl*>::AddResult addResult = stringTable.table().add<LCharBufferTranslator>(buffer);

    return addResult.isNewEntry ? adoptRef(*addResult.iterator) : *addResult.iterator;
}

PassRefPtr<StringImpl> AtomicString::addSlowCase(AtomicStringTable& stringTable, const UChar* s, unsigned length)
{
    if (!s)
        return 0;
    if (!length)
    ...
    AtomicStringTableLocker locker;
    HashSet<StringImpl*>::AddResult addResult = stringTable.table().add<UCharBufferTranslator>(buffer);

    return addResult.isNewEntry ? adoptRef(*addResult.iterator) : *addResult.iterator;
}

PassRefPtr<StringImpl> AtomicString::addSlowCase(AtomicStringTable& stringTable, const UChar* s)
{
    if (!s)
        return 0;
...
#if ENABLE(VMOLAB)
bool AtomicString::isInStringProviderTable(AtomicStringTable& stringTable, StringImpl* string)
{
    AtomicStringTableLocker locker;
    return stringTable.table().contains(string);
}
#endif
#endif
