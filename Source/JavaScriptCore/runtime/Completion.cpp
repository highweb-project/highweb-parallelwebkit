...
#if ENABLE(VMOLAB_TIME)
    TimeCounter::start(TimeCounter::TYPEeval);
    TimeCounter::start(TimeCounter::TYPEread);
#endif

#if ENABLE(VMOLAB)
    VM& vm = exec->vm();
    if (vm.isInParallelParseLoad() && !vm.isInParallelParseSave())
        vm.readAndAllocatePreParser(source); 
#endif
#if ENABLE(VMOLAB_TIME)
    TimeCounter::stop(TimeCounter::TYPEread);
#endif
...
#if ENABLE(VMOLAB_TIME)
    TimeCounter::stop(TimeCounter::TYPEeval);
#endif
