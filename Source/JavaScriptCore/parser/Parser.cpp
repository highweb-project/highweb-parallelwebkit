...
template <typename LexerType>
Parser<LexerType>::Parser(VM* vm, const SourceCode& source, FunctionParameters* parameters, const Identifier& name, JSParserStrictness strictness, JSParserMode parserMode, bool parallelMode = false)
    : m_vm(vm)
    , m_source(&source)
...
#if ENABLE(VMOLAB)
    , m_parserMode(parserMode)
    , m_isInParallelMode(parallelMode)
#endif
{
    m_lexer = adoptPtr(new LexerType(vm, strictness));
#if ENABLE(VMOLAB)
    m_arena = adoptPtr(new ParserArena);
    m_lexer->setCode(source, m_arena.get());
#else
...
String Parser<LexerType>::parseInner()
{
...
#if ENABLE(VMOLAB)
    ASTBuilder context(const_cast<VM*>(m_vm), const_cast<SourceCode*>(m_source), m_arena.get());
#else
    ...
    if (m_parserMode == JSParseFunctionCode) {
        if (!consume(OPENPAREN)) 
            ASSERT_NOT_REACHED();
        if (!match(CLOSEPAREN)) {
            parameterNodes = parseFormalParameters(context);
            ASSERT(parameterNodes);
        }
        ...
        RefPtr<FunctionParameters> parameters = FunctionParameters::create(parameterNodes);
        initializeParameters(parameters.get());
    }
#endif
...
#if ENABLE(VMOLAB)
template <typename LexerType>
void Parser<LexerType>::initializeParameters(FunctionParameters* parameters)
{
    ASSERT(parameters);
    ScopeRef scope = currentScope();
    bool hadBindingParameters = false;
...
template <class TreeBuilder> TreeFunctionBody Parser<LexerType>::parseFunctionBody(TreeBuilder& context)
{
    ...
    m_statementDepth = 0;
#if ENABLE(VMOLAB)
    typename TreeBuilder::FunctionBodyBuilder bodyBuilder(const_cast<VM*>(m_vm), m_arena.get());
#else
...
#if ENABLE(VMOLAB)
    if (const SourceProviderCacheItem* cachedInfo = m_isInParallelMode ? 0 : findCachedFunctionInfo(openBraceOffset)) {
#else
...
#if ENABLE(VMOLAB)
    if (!m_isInParallelMode && m_functionCache && functionLength > minimumFunctionLengthToCache) {
#else
