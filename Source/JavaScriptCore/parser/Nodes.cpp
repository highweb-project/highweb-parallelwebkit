...
#if ENABLE(VMOLAB)
    , ParserArenaRefCounted(vm, parserArena)
#else
...
inline ProgramNode::ProgramNode(VM* vm, 
#if ENABLE(VMOLAB)
        ParserArena* parserArena,
#endif
        const JSTokenLocation& startLocation, 
        ...
        int numConstants)
#if ENABLE(VMOLAB)
    : ScopeNode(vm, parserArena, startLocation, endLocation, source, children, varStack, funcStack, capturedVariables, features, numConstants)
#else
...
PassRefPtr<ProgramNode> ProgramNode::create(VM* vm, 
#if ENABLE(VMOLAB)
        ParserArena* parserArena,
#endif
        const JSTokenLocation& startLocation, 
...
{
#if ENABLE(VMOLAB)
    RefPtr<ProgramNode> node = new ProgramNode(vm, parserArena, startLocation, endLocation, startColumn, endColumn, children, varStack, funcStack,  capturedVariables, source, features, numConstants);
#else
...
inline EvalNode::EvalNode(VM* vm, 
#if ENABLE(VMOLAB)
        ParserArena* parserArena,
#endif
        const JSTokenLocation& startLocation, 
...
PassRefPtr<EvalNode> EvalNode::create(VM* vm, 
#if ENABLE(VMOLAB)
        ParserArena* parserArena,
#endif
        const JSTokenLocation& startLocation, 
...
{
#if ENABLE(VMOLAB)
    RefPtr<EvalNode> node = new EvalNode(vm, parserArena, startLocation, endLocation, endColumn, children, varStack, funcStack, capturedVariables, source, features, numConstants);
#else
...
#if ENABLE(VMOLAB)
inline FunctionBodyNode::FunctionBodyNode(VM* vm, ParserArena* parserArena, const JSTokenLocation& startLocation, const JSTokenLocation& endLocation, unsigned startColumn, unsigned endColumn, bool inStrictContext)
    : ScopeNode(vm, parserArena, startLocation, endLocation, inStrictContext)
    , m_startColumn(startColumn)
    , m_endColumn(endColumn)
{
}

inline FunctionBodyNode::FunctionBodyNode(VM* vm, ParserArena* parserArena, const JSTokenLocation& startLocation, const JSTokenLocation& endLocation, unsigned startColumn, unsigned endColumn, SourceElements* children, VarStack* varStack, FunctionStack* funcStack, IdentifierSet& capturedVariables, const SourceCode& sourceCode, CodeFeatures features, int numConstants)
    : ScopeNode(vm, parserArena, startLocation, endLocation, sourceCode, children, varStack, funcStack, capturedVariables, features, numConstants)
    , m_startColumn(startColumn)
    , m_endColumn(endColumn)
{
}
...
#if ENABLE(VMOLAB)
void FunctionBodyNode::finishParsing(const Identifier& ident, enum FunctionMode functionMode)
{
    ASSERT(!source().isNull());
    m_ident = ident;
    m_functionMode = functionMode;
...
#if ENABLE(VMOLAB)
FunctionBodyNode* FunctionBodyNode::create(VM* vm, ParserArena* parserArena, const JSTokenLocation& startLocation, const JSTokenLocation& endLocation, unsigned startColumn, unsigned endColumn, bool inStrictContext)
{
    return new FunctionBodyNode(vm, parserArena, startLocation, endLocation, startColumn, endColumn, inStrictContext);
}

PassRefPtr<FunctionBodyNode> FunctionBodyNode::create(VM* vm, ParserArena* parserArena, const JSTokenLocation& startLocation, const JSTokenLocation& endLocation, unsigned startColumn, unsigned endColumn, SourceElements* children, VarStack* varStack, FunctionStack* funcStack, IdentifierSet& capturedVariables, const SourceCode& sourceCode, CodeFeatures features, int numConstants)
{
    RefPtr<FunctionBodyNode> node = new FunctionBodyNode(vm, parserArena, startLocation, endLocation, startColumn, endColumn , children, varStack, funcStack, capturedVariables, sourceCode, features, numConstants);
