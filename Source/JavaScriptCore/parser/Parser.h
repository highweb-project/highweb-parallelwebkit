...
public:
#if ENABLE(VMOLAB)
    Parser(VM*, const SourceCode&, FunctionParameters*, const Identifier&, JSParserStrictness, JSParserMode, bool parallelMode);
#else
...
#if ENABLE(VMOLAB)
    void initializeParameters(FunctionParameters* parameters);
#endif
...
#if ENABLE(VMOLAB)
        Parser<Lexer<LChar>> parser(vm, source, parameters, name, strictness, parserMode, parallelMode);
#else
...
#if ENABLE(VMOLAB)
    Parser<Lexer<UChar>> parser(vm, source, parameters, name, strictness, parserMode, parallelMode);
#else
